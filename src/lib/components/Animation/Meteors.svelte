...
{#each meteorStyles as style, idx}
	<span
		id="meteor-{idx + 1}"
		class={cn(
			'animate-meteor pointer-events-none absolute top-1/2 left-1/2 size-[4px] rotate-[215deg] rounded-full bg-yellow-400 shadow-[0_0_6px_2px_rgba(255,215,0,0.8)]',
		)}
	...
	>
		...
		<div
			class="pointer-events-none absolute top-1/2 -z-10 h-px w-[80px] -translate-y-1/2 bg-gradient-to-r from-yellow-300 via-yellow-400/70 to-transparent"
		/>
	</span>
{/each}
